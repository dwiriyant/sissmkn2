...
var $table3 = 't_siswa';
  ...
  function tampilsiswa($no_induk, $tglabsen){
    ...
    function filterjumlahsiswaperkelas($id,$tglabsen){
    $con = $this->dbconnect();
    $sql = "SELECT * FROM t_absensi 
        LEFT JOIN t_siswa on t_siswa.no_induk = t_absensi.no_induk
        LEFT JOIN t_kelas on t_kelas.id_kelas = t_absensi.id_kelas
        WHERE t_absensi.id_kelas = '$id' and t_absensi.tglabsen='$tglabsen'";
   ...
   function tampilidabsenrfid() {
      $con = $this->dbconnect();
      $sql = 'SELECT t_siswa.no_induk, t_siswa.nama, t_kelas.kelas, t_absensi.datang, t_absensi.pulang, t_siswa.foto_daftar from t_absensi
              left join t_siswa on t_siswa.no_induk = t_absensi.no_induk
