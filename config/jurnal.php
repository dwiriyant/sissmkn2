...
class jurnal extends Database
{
	...
	{
		$con = $this->dbconnect();
		$sql = 'INSERT INTO '.$this->tabel.' VALUES("'.$id_jurnal.'", "'.$userid.'", "'.$thajar.'", "'.$nama.'", "'.$hari.'", "'.$tgl.'", "'.$kelas.'", "'.$jam_mulai.'", "'.$jam_selesai.'", "'.$kegiatan.'", "'.$sdh_blm.'", "'.$ket.'")';
	...
	{
		$con = $this->dbconnect();
		$sql = "SELECT *, a.nip, b.nama, b.hari, b.tgl, b.kelas, b.jam_mulai, b.kegiatan 
		from t_jurnal as b, t_staf as a WHERE b.nama = a.nama GROUP BY b.id_jurnal LIMIT 0,50" ;
		$query = mysqli_query($con,$sql);
		while($item = mysqli_fetch_array($query)){
			$hasil[] = $item;
		}
